...
function compute_features_selected_deploy(input_mat)
...
for i = 1:size(EEG.data,3) already_computed_feats{i} = []; already_computed_feats{i}.name = ''; end
...
if ~isempty(currEpochs_finished) epochs_to_process = setdiff(1:size(EEG.data,3),currEpochs_finished(currFeatures_dir_OK)); end
