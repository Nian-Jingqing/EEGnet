function [arr_sortamp, arr_plotamp, arr_avgamp] = phase_amplitude_coupling_function_custom_bichannel(dataset,higher_freq_idx,lower_freq_idx,PAC_opt)
...
try    
    ...
    arr_sortamp = zeros(numberOfBin, 2, segment_num,size(channels_pac,1),size(channels_pac,1));
    % arr_ch_sortamp = zeros(numberOfBin, 2, segment_num, length(channels_pac));
    arr_plotamp = zeros(numberOfBin,segment_num,size(channels_pac,1),size(channels_pac,1));
    arr_avgamp = zeros(segment_num,size(channels_pac,1),size(channels_pac,1));
    arr_finalavgamp = zeros(size(channels_pac,1),size(channels_pac,1));
    ...
    for segment = 1:segment_num
        ...
        for n1 = 1:size(channels_pac,1)    % Find average over all channels
            
            % Put in another loop here to loop through other channels
            % SAURABH
            
            %%%%%%%%%%%%%% TO DO: MAKE MEMORY EFFICIENT BY NOT INDEXING THE
            %%%%%%%%%%%%%% DIFFERNET CHANNELS UNTIL THE VERY END IN
            %%%%%%%%%%%%%% ARR_SORTAMP,ETC.
            
            for n2 = 1:size(channels_pac,1)    % Find average over all channels
...
%         end
        ...
        avgchsortamp = zeros(numberOfBin,n1,n2);
        for i = 1:numberOfBin
            if ch_sortamp(i,2,n1,n2) == 0
                avgchsortamp(i,n1,n2) = 0;
            else
                avgchsortamp(i,n1,n2) = (ch_sortamp(i,1,n1,n2)/ch_sortamp(i,2,n1,n2));
            end
        end
...
%         plotamp = zeros(1,numberOfBin);
        ...
        avgchamp = mean(avgchsortamp,1);
        plotchamp = zeros(numberOfBin,n1,n2);
        
        %For each bins set the value at that position
        for i = 1:numberOfBin
            plotchamp(i,n1,n2) = (avgchsortamp(i,n1,n2)-avgchamp(n1,n2))/avgchamp(n1,n2) + 1;
        end

        plotchamp(:,n1,n2) = plotchamp(:,n1,n2) - 1;            % Do this because median filter assumes 0 on each side
        plotchamp(:,n1,n2) = medfilt1(plotchamp(:,n1,n2), 2);   % January 16, 2014
        plotchamp(:,n1,n2) = plotchamp(:,n1,n2) + 1;
...
%         end
        ...
        arr_sortamp(:,:,segment,n1,n2) = squeeze(ch_sortamp(:,:,n1,n2));
        arr_plotamp(:,segment,n1,n2) = squeeze(plotchamp(:,n1,n2));
        arr_avgamp(segment,n1,n2) = squeeze(avgchamp(n1,n2));
        arr_finalavgamp(n1,n2) = squeeze(mean(avgchamp(n1,n2),1));
            end
        end
